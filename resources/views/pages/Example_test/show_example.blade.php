...
<div class="all-title-box">
    <div class="container text-center">
        <h1> Test <span class="m_1">Lorem Ipsum dolroin gravida nibh vel velit.</span></h1>
    </div>
</div>
...
<script>
    @if(isset($_GET['timer']))
        ...
        if (sessionStorage.getItem('timer') === null) {
            sessionStorage.setItem('timer', "{{ $_GET['timer'] }}");

            str = window.location.href
            window.history.pushState('state','title',str.replace('&timer={{ $_GET['timer'] }}',''))
        }
    
    @endif
</script>

@if($filter_status == 2 || $filter_status == 0 || $lesson_status != 0)
...
@elseif($filter_status != 2 && $filter_status != 0 && $lesson_status == 0)
<div id="timeANDshow">
    ...
    <script>
                ...
                if (sessionStorage.getItem('timer') !== null) {
                    ...
                    var listen = parseInt({{$amountquesListen}}*1.142857142857143) * 60;
                    var read = parseInt({{$amountquesread}}*1.125) * 60;
                    var write =  parseInt({{$amountqueswrite}}*30) * 60;
                    var speak = parseInt({{$amountquesspeak}}*1.2) * 60;

                    //listen
                    var timer = new Date((parseInt(sessionStorage.getItem('timer')) + @if($filter_status != 3) (listen + read + write + speak) @else 30 * 60 @endif) * 1000).getTime();
                    
                    var x = setInterval(function(){

                        var distance = timer - new Date().getTime();

                        var h = Math.floor(distance % (1000 * 60 * 60 * 24) / (1000 * 60 * 60 * 1));
                        var m = Math.floor(distance % (1000 * 60 * 60) / (1000 * 60 * 1));
                        var s = Math.floor(distance % (1000 * 60) / (1000 * 1));
        ...
        <form id="result_level" data-result_id="{{ $result_id }}" action="{{ URL::to('/results-example-test/'.$result_id) }}" method="post">
                                    ...
                                    <div class="mcq-wrapper">
                                        ...
                                        <div class="options-wrapper">                                    
                                            ...
                                            <div class="option">
                                                ...
                                                <input value="{{ $answers->answer_id }}" type='radio' class="count_ques_read" data-id_question="{{ $all_ques_read['question_id'] }}" name="nameRead[{{ $all_ques_read['question_id'] }}]" data-no-ques="{{ $i - 1 }}" data-skill="read" data-answer="{{ $ans1[$index] }}" id="id[{{  $answers->answer_id  }}]"  />
                                        ...
                                        <div class="card-footer py-3 border-0" style="background-color: #f8f9fa;">
                                            <div class="d-flex flex-start w-100">
                                              ...
                                              <div class="form-outline w-100">
                                                <textarea name="nameWrite[{{$all_ques_write['question_id']}}]" data-id_question="{{$all_ques_write['question_id']}}" class="form-control " rows="4" style="background: #fff;" ></textarea>
                            ...
                            <div style="margin-top: 10px">
                                ...
                                <div class="reading-text-wrapper text-en">
                                    <div class="paragraph-p7 topic " style="text-align:center;">
                                        ...
                                        <p style="text-align:left" class="reading-text-paragraph" ><span class="paragraph-sentence"> {!! $all_topic_speak['topic_content'] !!} </span></p>
                                    ...
                                    <div class="mcq-wrapper">
                                        ...
                                        <br>
                                        <span>Click the record button to record</span>

                                        <span id="recording_{{$all_ques_speak['question_id']}}" style="color:red; display:none;">Recording...</span>
                                        <br><audio  class="audiosave"  id="{{$all_ques_speak['question_id']}}" name="nameSpeak[{{$all_ques_speak['question_id']}}]"></audio>
