	...
	<div class="forms-grids">
		<div class="w3agile-validation">
			...
			<div class="panel panel-widget agile-validation">
				<div class="my-div">
					<form id="form_update" method="post" action="{{URL::to('/admin/update-route-control/'.$route->route_id)}}" >
						@csrf
						...
						<div class="form-group">
							<input type="text" name="route_name" value="{{ $route->route_name }}" class="form-control">
							<span class="error_mess"></span>
						</div>
						<label for="field-2">Description: <span class="at-required-highlight">*</span></label>
						<div class="form-group">
							<textarea name="route_des">{!! $route->route_des !!}</textarea>
							<span class="error_mess"></span>
						</div>
						...
						<div class="route">
							...
							@foreach($data as $key => $detail)
								<div class="count">
									<label for="field-2">Structure {{ $key+1 }}: <span class="at-required-highlight">*</span></label>
									<div class="form-group">
										<select name="filter_id[]" class="form-control" @if($key != 0) style="width: 96%;display: inline;" @endif>
											<option value="-1" disabled>Chose Structure here</option>
											@foreach($filters as $filter)
												<option value="{{ $filter['filter_id'] }}" @if($detail['filter_id'] == $filter['filter_id']) selected @endif>{{ $filter['filter_name'] }}</option>
											@endforeach
										</select>

										@if($key != 0)
											<button type="button" data-filter="{{ $detail['filter_id'] }}" data-id="{{ $route->route_id }}" class="btn-sm btn-warning delBtn" style="background-color: orange;border: none;"><i class="glyphicon glyphicon-remove"></i></button>
										@endif

										<span class="error_mess"></span>
									</div>
									<button type="button" class="btn btn-primary detailBtn" style="background-color: #666;">Details</button><br>
								</div>
							@endforeach
			...
			$(document).ready(function(){
						...
						$('.route').append(`
							<div class="count">
								...
								<div class="form-group">
									...
									<button type="button" class="btn-sm btn-warning delBtn" style="background-color: red;border: none;"><i class="glyphicon glyphicon-remove"></i></button>
									<span class="error_mess"></span>
								</div>
				...
				$(document).on('click','.delBtn',function(){
					...
					if ($(this).css('background-color') == 'rgb(255, 0, 0)') {
						...
						Swal.fire({
							icon: 'question',
							title: 'Do you want to remove this Structure ?',
							showConfirmButton: true,
							confirmButtonColor: '#00bcd4',
							showDenyButton: true,
							backdrop: false,
							...
							if (result.isConfirmed) {
								var id = $(selector_this).data('id');
								var filter = $(selector_this).data('filter');

								window.location = "{{ URL::to('/admin/delete-route-struct') }}/" + id + '?filter_id=' + filter;
							}
						})
					}
				});
